...
export default async function CoursePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="course-page" style={{ "--course-accent": course.color } as CSSProperties}>
    ...
    {course.teachers.length > 0 && <section className="course-section shell"><span className="eyebrow">Команда</span><h2>Викладачі курсу</h2><div className="course-detail-grid course-teacher-grid">{course.teachers.map((teacher) => <article className="content-card course-teacher" key={teacher.name}>{teacher.photoUrl && <Image className="course-teacher__photo" src={teacher.photoUrl} alt={`Тренерка ${teacher.name}`} width={280} height={280}/>}<h3>{teacher.name}</h3><p><strong>{teacher.role}</strong></p><p>{teacher.bio}</p></article>)}</div></section>}
